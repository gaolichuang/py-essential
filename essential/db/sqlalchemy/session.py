...
from essential.gettextutils import _LE, _LW
    ...
    def _do_query(self, q):
        ...
        for filename, line, method, function in traceback.extract_stack():
            ...
            if filename.endswith('db/api.py'):
                continue
            # only trace inside essential
            index = filename.rfind('essential')
            if index == -1:
                continue
...
class EngineFacade(object):
    """helper class for removing of global engine instances from essential.db.

    As a library, essential.db can't decide where to store/when to create
    engine and sessionmaker instances, so this must be left for a target
    application.

    On the other hand, in order to simplify the adoption of essential.db
    ...
    def from_config(cls, connection_string, conf,
        ...
        """Initialize EngineFacade using essential.config config instance
        options.

        :param connection_string: SQLAlchemy connection string
        :type connection_string: string

        :param conf: essential.config config instance
        :type conf: essential.config.cfg.ConfigOpts
